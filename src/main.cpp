...
void enterDeepSleep(bool fromTimeout = false) {
  ...
  APP_STATE.sleepingFromDashboard = activityManager.isDashboardActivity();
  ...
  // Drop any BLE HID link cleanly so the keyboard/remote sees the disconnect promptly.
...
// while a reader (keyboard/page-turner input) or the Bluetooth settings screen
// (pairing) is on the stack AND WiFi is off. Heap-heavy reader phases may stop BLE
// directly; this lifecycle then restarts it only after the normal
// activity/render/heap gates pass.
void updateBluetoothLifecycle() {
  ...
  // dead without the stack). Its floor only needs to cover NimBLE itself — the reader
  // floor reserves build/render headroom that never gets used there.
  ...
  if (wanted && !BleHid.isRunning()) {
    // Back off after a failed begin(): NimBLE init failures are not transient at
    // loop rate, and retrying every ~10 ms would spin the CPU and flood the log.
    static uint32_t nextStartAttemptMs = 0;
    if (millis() < nextStartAttemptMs) return;
    ...
    if (!bleinput::ensureStarted()) {
      LOG_ERR("BLELC", "start failed heap=%u maxAlloc=%u; backing off 5s", ESP.getFreeHeap(), ESP.getMaxAllocHeap());
      nextStartAttemptMs = millis() + 5000;
      return;
    }
